...
{
	return GL_Vector3(u.x*s, u.y*s, u.z*s);
}
...
GL_Vector3  Cross(GL_Vector3 vVector1, GL_Vector3 vVector2)
{
	...
	return GL_Vector3((vVector1.y * vVector2.z) - (vVector1.z * vVector2.y),
...
{														
	...
	//注意它的次序反了lygon[1] - vPolygon[0];
	//              GL_Vector3 vVector2 = vPolygon[2] - vPolygon[0];    
	
	GL_Vector3 vVector1 = vPolygon[2] - vPolygon[0];
	GL_Vector3 vVector2 = vPolygon[1] - vPolygon[0];

	GL_Vector3 vNormal = Cross(vVector1, vVector2);		// Take the cross product of our 2 vectors to get a perpendicular vector
...
GL_Vector3  ClosestPointOnLine(GL_Vector3 vA, GL_Vector3 vB, GL_Vector3 vPoint)
{
	...
	GL_Vector3 vVector1 = vPoint - vA;

	// Create a normalized direction vector from end point vA to end point vB
    GL_Vector3 vVector2 = Normalize(vB - vA);
...
//////////////////////// PLANE DISTANCE \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
////
////	This returns the distance between a plane and the origin(0, 0, 0)
////
//////////////////////// PLANE DISTANCE \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
//这个也可以看看dot_product，点积的意义									
...
{
	GL_Vector3 vPoint, vLineDir;					// Variables to hold the point and the line's direction
	...
	// 为了和下面的分母相匹配，分子部分应添个符号
	// 因为那个distance是加了负号的(-dot())，所以前面再加个负号
...
bool  InsidePolygon(GL_Vector3 vIntersection, GL_Vector3 Poly[], long verticeCount)
{
	...
	GL_Vector3 vA, vB;						// Create temp vectors
...
bool  IntersectedPolygon(GL_Vector3 vPoly[], GL_Vector3 vLine[], int verticeCount)
{
	...
	GL_Vector3 vIntersection = IntersectionPoint(vNormal, vLine, originDistance);
...
/** 判断视点球体与有向面片的位置关系，计算距离**/
int  ClassifySphere(GL_Vector3 &vCenter,GL_Vector3 &vNormal, GL_Vector3 &vPoint, float radius, float &distance)
{
	// 计算平面与原点的距离d
							...
							GL_Vector3 &vCenter, int vertexCount, float radius)
{
	...
	GL_Vector3 vNormal = Normal(vPolygon);
	...
	if(classification == INTERSECTS) 
	{
		...
		GL_Vector3 vPosition = vCenter - vOffset;
...
{
	GL_Vector3 vOffset = GL_Vector3(0, 0, 0);
