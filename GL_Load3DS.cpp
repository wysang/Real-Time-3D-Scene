...
C3DSModel::C3DSModel()
{
	...
	strcpy_s(defaultMat.matName.string, "me");
...
C3DSModel::~C3DSModel()
{
	//Release();   这个放在class Me类的成员函数Deinitialize（）里执行释放空间。
	m_3DModel.pMaterials.clear();
	m_3DModel.pObject.clear();
}

// 载入3ds文件
BOOL C3DSModel::Load(char *dir,char *strFileName)
{
	char	appdir[256];
	...
	for (int i=0; i<m_3DModel.numOfMaterials; i++)
	{
		...
		{
			if (!m_texture.MakeTextureBind(m_3DModel.pMaterials[i].mapName.string, m_3DModel.pMaterials[i].texureId))
			{
				// 纹理载入失败
...
UINT C3DSModel::ReadObjectInfo(t3DObject *pObj, UINT n)
{
	...
	while (count < n)
	{
		...
		case OBJECT_VERTEX:
			...
			pObj->pVerts = new GL_Vector3[pObj->numOfVerts];
			memset(pObj->pVerts, 0, sizeof(GL_Vector3) * pObj->numOfVerts);
			// 按块读取顶点坐标值
		...
		case OBJECT_UV:
			...
			pObj->pTexVerts = new Texcoord[pObj->numTexVertex];
			memset(pObj->pTexVerts, 0, sizeof(Texcoord) * pObj->numTexVertex);
			// 按块读取纹理坐标值
...
{
	GL_Vector3 v1,v2, vNormal,vPoly[3];
	...
	{
		obj = &m_3DModel.pObject[nOfObj];
		GL_Vector3 *pNormals		= new GL_Vector3 [obj->numOfFaces];
		GL_Vector3 *pTempNormals	= new GL_Vector3 [obj->numOfFaces];
		obj->pNormals			= new GL_Vector3 [obj->numOfVerts];
		...
		GL_Vector3 vSum(0.0, 0.0, 0.0);
		GL_Vector3 vZero(0.0, 0.0, 0.0);
		int shared=0;
...
void C3DSModel::Scene(float x,float y,float z,float size ,float angle,float dx,float dy,float dz)
{
		glPushMatrix();
	    glTranslatef(x,y,z);  //沿各个方向平移  高度为当前地面高度
		glScalef(size,size,size);  //分别研三个方向放大到原来的0.1倍
